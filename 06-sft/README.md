...
SFT（Supervised Fine-Tuning）用高质量输入—输出样本训练模型，是后续蒸馏和强化学习实验的重要基线。
...
| 顺序 | 学习内容 | 建议代码文件 | 完成标准 |
| ---: | --- | --- | --- |
| 01 | SFT 数据格式 | `lessons/01-sft-dataset.py` | 能检查 messages、角色、空回答和重复样本 |
| 02 | Chat Template | `lessons/02-format-and-tokenize.py` | 能展示模板化文本、Token 和标签 |
| 03 | Loss Mask | `lessons/03-label-masking.py` | 能说明哪些 Token 参与损失计算 |
| 04 | LoRA 原理 | `lessons/04-lora.py` | 能解释 `r`、`alpha`、target modules 和可训练参数量 |
| 05 | SFTTrainer | `lessons/05-sft-trainer.py` | 在极小数据集上跑通训练与保存 |
| 06 | QLoRA | `lessons/06-qlora.py` | 理解 4-bit 基座、计算 dtype 与 Adapter 的关系 |
| 07 | 训练评测 | `lessons/07-evaluate-adapter.py` | 加载 Adapter，并用同一评测器比较基线 |
