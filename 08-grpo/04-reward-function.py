import re
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer

model_name = "Qwen/Qwen2.5-0.5B-Instruct"

device = torch.device(
    "cuda" if torch.cuda.is_available()
    else "cpu"
)

tokenizer = AutoTokenizer.from_pretrained(
    model_name,
)

model = AutoModelForCausalLM.from_pretrained(
    model_name,
    dtype = torch.bfloat16
)

model = model.to(device)
model.eval()
question = """
Janet has 12 apples.
She gives 3 apples to each of 2 friends.
How many apples does she have left?
""".strip()
messages = [
    {
        "role": "system",
        "content": (
            "Solve the math problem step by step. "
            "At the end, output the final numerical answer "
            "in exactly this format: #### <answer>"
        )
    },
    {
        "role": "user",
        "content": question
    }
]

prompt = tokenizer.apply_chat_template(
    messages,
    tokenize = False,
    add_generation_prompt = True
)

inputs = tokenizer(
    prompt,
    return_tensors = "pt"
).to(device)

# print(prompt)
# print(
#     "input_ids shape : ",
#     inputs["input_ids"].shape
# )
G = 4
with torch.inference_mode():
    outputs = model.generate(
        **inputs,
        do_sample = True,
        temperature = 0.9,
        top_p = 0.95,
        num_return_sequences = G,
        max_new_tokens = 256,
        pad_token_id = tokenizer.eos_token_id
    )

prompt_length = inputs["input_ids"].shape[1]
rewards = []
ground_truth = "6"
def extract_strict_answer(text):
    pattern = r"####\s*(-?\d+(?:\.\d+)?)\s*$"

    match = re.search(
        pattern,
        text.strip()
    )

    if match is None:
        return None

    return match.group(1)

def format_reward(text):

    answer = extract_strict_answer(text)

    if answer is None:
        return 0.0
    return 1.0

def extract_answer(text):

    # 1. 最优先：#### 6
    strict = extract_strict_answer(text)

    if strict is not None:
        return strict


    # 2. 尝试 \boxed{6}
    boxed = re.findall(
        r"\\boxed\{(-?\d+(?:\.\d+)?)\}",
        text
    )

    if boxed:
        return boxed[-1]


    # 3. 教学版 fallback：
    # 取文本中最后出现的数字
    numbers = re.findall(
        r"-?\d+(?:\.\d+)?",
        text
    )

    if numbers:
        return numbers[-1]

    return None

def accuracy_reward(
    text,
    ground_truth
):
    pred = extract_answer(text)

    if pred is None:
        return 0.0
    if pred == ground_truth:
        return 1.0
    
    return 0.0

# def total_reward(text, ground_truth):

#     acc = accuracy_reward(text, ground_truth)
#     fmt = format_reward(text)

#     reward = (1.0 * acc + 0.1 * fmt)

#     return reward

for i, output_ids in enumerate(outputs):

    completion_ids = output_ids[prompt_length:]

    completion = tokenizer.decode(
        completion_ids,
        skip_special_tokens = True
    )

    pred = extract_answer(completion)

    acc_reward = accuracy_reward(completion, ground_truth)
    fmt_reward = format_reward(completion)
    reward = (acc_reward * 1.0 + fmt_reward * 0.1)
    rewards.append(reward)

    print(
        f"\n==== Rollout {i + 1} ===="
    )

    print(completion)

    print("Extracted:", pred)

    print("Accracy reward:", acc_reward)

    print("Format reward : ", fmt_reward)

    print("Total reward :", reward)
print("\nRewards:", rewards)